...
public class QRBarcodeGen {
	 ...
	 */
	public static String createQrCode(String content, int qrCodeSize, String imageFormat) {
...
//			 graphics.drawString( content, 30, image.getHeight() -
//			 graphics.getFont().getSize());
			...
			logo = getScaledImage(logo, (int) (logo.getWidth() * scale),
					(int) (logo.getHeight() * scale));
			graphics.drawImage(logo,
					image.getWidth() / 2 - logo.getWidth() / 2,
					image.getHeight() / 2 - logo.getHeight() / 2,
					image.getWidth() / 2 + logo.getWidth() / 2,
					image.getHeight() / 2 + logo.getHeight() / 2, 0, 0,
					logo.getWidth(), logo.getHeight(), null);
	...
	private static boolean isQRCodeCorrect(String content, BufferedImage image) {
		...
		if (qrResult != null && content != null
				&& content.equals(qrResult.getText())) {
			result = true;
		}
		return result;
	}
	 ...
	 * @throws IOException
	 */
	private static BufferedImage getScaledImage(BufferedImage image, int width,
