var SayCheese = (function() {
	...
	SayCheese = function SayCheese(element, options) {
		this.snapshots = [], this.video = null, this.events = {},
				this.stream = null, this.options = {
					videoSource : null,
					snapshots : true,
					audio : false,
					width : 320
	...
	SayCheese.prototype.createVideo = function createVideo() {
		...
		var width = this.options.width, height = 0, streaming = false;
	...
	SayCheese.prototype.takeSnapshot = function takeSnapshot(width, height) {
		...
		var snapshot = document.createElement('canvas'), ctx = snapshot
				.getContext('2d');
